...
from siamese_sts.dataLoader import SiameseDataset
...
def get_data_loader(datasets,batch_size=64):

    sts_dataloaders = {}

    for j in range(0, 2):
        split_name = "train_loader" if j == 0 else "test_loader"

        sts_dataset = SiameseDataset(datasets[j])
...
def train_model(model, optimizer, dataloader, data, max_epochs, config_dict):
    ...
    for i in range(10):
        ...
        sub_dataLoader = get_data_loader(dataset)
        for epoch in tqdm(range(max_epochs)):
            ...
            ) in sub_dataLoader['train_loader']:
            ...
            ## compute model metrics on dev set  改用测试的
            val_acc, val_loss = evaluate_dev_set(
                model, data, criterion, sub_dataLoader, config_dict, device
            )
